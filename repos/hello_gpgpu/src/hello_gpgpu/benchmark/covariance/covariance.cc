...
void cl_load_prog()
{
	...
	const size_t kernel_size = covariance_Gen9core_gen_len;
    const unsigned char* kernel_bin = covariance_Gen9core_gen;
...
void cl_clean_up()
{
	...
	//errcode = clReleaseKernel(clKernel_std); // this kernel is never created and can therefore not be released
	...
	//errcode = clReleaseMemObject(stddev_mem_obj); // this memobject is never allocated and can therefore not be released
