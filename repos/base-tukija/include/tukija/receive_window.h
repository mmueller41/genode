...
struct Genode::Receive_window
{
	...
	public:
		 ...
		 */
		bool prepare_rcv_window(Tukija::Utcb &utcb,
		 ...
		 */
		void post_ipc(Tukija::Utcb &utcb, addr_t const rcv_window = INVALID_INDEX)
		{
			...
			for (unsigned i = 0; i < rcv_items; i++) {
				Tukija::Utcb::Item * item = utcb.get_item(i);
				if (!item)
					break;

				Tukija::Crd cap(item->crd);
