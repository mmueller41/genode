                                 ...
                                 Genode::addr_t const sel, Genode::addr_t event)
{
	...
	Tukija::Crd orig_crd = utcb->crd_rcv;

	/* request event-handler portal */
	utcb->crd_rcv = Tukija::Obj_crd(sel, 0);
	utcb->msg()[0]  = event;
	utcb->set_msg_word(1);

	Genode::uint8_t res = Tukija::call(cap);
                                   ...
                                   Genode::addr_t const sel)
{
	...
	Genode::uint8_t res = Tukija::NOVA_OK;
	enum {
