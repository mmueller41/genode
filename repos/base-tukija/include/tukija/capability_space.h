...
namespace Genode { namespace Capability_space {
	...
	using Ipc_cap_data = Tukija::Crd;

	static inline Tukija::Crd crd(Native_capability const &cap)
	{
		/*
		 * We store the 'Nova::Crd' value in place of the 'Data' pointer.
		 */
		addr_t value = (addr_t)cap.data();
		Tukija::Crd crd = *(Tukija::Crd *)&value;
		return crd;
	}

	static inline Native_capability import(addr_t sel, unsigned rights = 0x1f)
	{
		Tukija::Obj_crd const crd = (sel == INVALID_INDEX)
		                        ? Tukija::Obj_crd() : Tukija::Obj_crd(sel, 0, rights);
