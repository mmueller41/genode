...
static Tukija::uint8_t map_async_caps(Tukija::Obj_crd const src,
                                    Tukija::Obj_crd const dst,
                                    addr_t const dst_pd)
{
	using Tukija::Utcb;
	...
	return Tukija::delegate(src_pd, dst_pd, dst);
}


static Tukija::uint8_t kernel_quota_upgrade(addr_t const pd_target)
{
                                          ...
                                          auto const &fn)
{
	...
	do {
		res = fn();
	} while (res == Tukija::NOVA_PD_OOM &&
	         Tukija::NOVA_OK == kernel_quota_upgrade(pd_target));
	return res;
}
...
Core::Trace::Source::Info Vm_session_component::Vcpu::trace_source_info() const
{
	...
	uint8_t res = Tukija::sc_ec_time(sc_sel(), ec_sel(), sc_time, ec_time);
	...
	return { _label, String<5>("vCPU"),
	         Trace::Execution_time(ec_time, sc_time,
	                               Tukija::Qpd::DEFAULT_QUANTUM, _priority),
	         _location };
}
...
void Vm_session_component::Vcpu::startup()
{
	...
	uint8_t res = _with_kernel_quota_upgrade(_pd_sel, [&] {
		return Tukija::create_sc(sc_sel(), _pd_sel, ec_sel(),
		                       Tukija::Qpd(Tukija::Qpd::DEFAULT_QUANTUM, _priority));
	});
                                              ...
                                              Signal_context_capability const cap)
{
	...
	if (exit >= Tukija::NUM_INITIAL_VCPU_PT)
		return;

	/* map handler into vCPU-specific range of VM protection domain */
	addr_t const pt = Tukija::NUM_INITIAL_VCPU_PT * _id + exit;

	uint8_t res = _with_kernel_quota_upgrade(_pd_sel, [&] {
		Tukija::Obj_crd const src(cap.local_name(), 0);
		Tukija::Obj_crd const dst(pt, 0);
	...
	_trace_control_slot(trace_control_area)
{
	...
	/* setup resources */
	uint8_t res = _with_kernel_quota_upgrade(_pd_sel, [&] {
		return Tukija::create_sm(sm_sel(), core_pd_sel, 0);
	});
	...
	addr_t const event_base = (1U << Tukija::NUM_INITIAL_VCPU_PT_LOG2) * id;
	enum { THREAD_GLOBAL = true, NO_UTCB = 0, NO_STACK = 0 };
	res = _with_kernel_quota_upgrade(_pd_sel, [&] {
		return Tukija::create_ec(ec_sel(), _pd_sel, kernel_id,
	...
	addr_t const dst_sm_ec_sel = Tukija::NUM_INITIAL_PT_RESERVED + _id*CAP_RANGE;
                                             ...
                                             Attach_attr const attribute)
{
	...
	while (page.valid()) {
		Tukija::Rights const map_rights (true,
		                               dsc.writeable() && attribute.writeable,
		                               attribute.executable);
		Tukija::Mem_crd const mem(page.addr >> 12, page.log2_order - 12,
		                        map_rights);
		...
		Tukija::Mem_crd crd_mem(page.hotspot >> 12, page.log2_order - 12,
		                      map_rights);

		/* asynchronously map memory */
		uint8_t res = _with_kernel_quota_upgrade(_pd_sel, [&] {
			return Tukija::delegate(src_pd, _pd_sel, crd_mem); });
...
void Vm_session_component::_detach_vm_memory(addr_t guest_phys, size_t size)
{
	...
	while (page.valid()) {
		Tukija::Mem_crd mem(page.addr >> 12, page.log2_order - 12, revoke_rwx);
		Tukija::revoke(mem, true, true, _pd_sel);
	...
	_session_label(label)
{
	...
	uint8_t res = Tukija::create_pd(_pd_sel, core_pd, Tukija::Obj_crd(),
