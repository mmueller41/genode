                                  ...
                                  Tukija::Utcb &utcb)
{
	...
	using Tukija::NUM_INITIAL_PT_LOG2;
...
bool Platform_thread::_create_and_map_oom_portal(Tukija::Utcb &utcb)
{
	...
	addr_t const source_pd = platform_specific().core_pd_sel();
	return !map_local(source_pd, utcb, Tukija::Obj_crd(pt_oom, 0),
	                  Tukija::Obj_crd(_sel_pt_oom(), 0));
}
...
Trace::Execution_time Platform_thread::execution_time() const
{
	...
	if (!sc_created()) {
		...
		uint8_t res = Tukija::ec_time(_sel_ec(), ec_time);
		if (res != Tukija::NOVA_OK)
			warning("ec_time failed res=", res);
		return { ec_time, sc_time, Tukija::Qpd::DEFAULT_QUANTUM, _priority };
	}

	uint8_t res = Tukija::sc_ec_time(_sel_sc(), _sel_ec(), sc_time, ec_time);
	if (res != Tukija::NOVA_OK)
