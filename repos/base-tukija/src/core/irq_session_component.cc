...
static bool irq_ctrl(addr_t irq_sel, addr_t &msi_addr, addr_t &msi_data,
                     addr_t sig_sel, Tukija::Gsi_flags flags, addr_t virt_addr)
{
	/* assign IRQ to CPU && request msi data to be used by driver */
	uint8_t res = Tukija::assign_gsi(irq_sel, virt_addr, boot_cpu(),
...
static void deassociate(addr_t irq_sel)
{
	...
	if (!irq_ctrl(irq_sel, dummy1, dummy2, irq_sel, Tukija::Gsi_flags(), 0))
...
{
	if (!phys_mem)
		return irq_ctrl(irq_sel, msi_addr, msi_data, sig_cap.local_name(), Tukija::Gsi_flags(), 0);
	...
	return platform().region_alloc().alloc_aligned(4096, 12).convert<bool>(
		...
		[&] (void *virt_ptr) {
			...
			Tukija::Mem_crd phys_crd(phys_mem  >> 12, 0, Rights(true, false, false));
			Tukija::Mem_crd virt_crd(virt_addr >> 12, 0, Rights(true, false, false));
			...
			bool res = irq_ctrl(irq_sel, msi_addr, msi_data, sig_cap.local_name(), Tukija::Gsi_flags(), virt_addr);

			unmap_local(Tukija::Mem_crd(virt_addr >> 12, 0, Rights(true, true, true)));
			platform().region_alloc().free(virt_ptr, 4096);
...
{
	if (Tukija::NOVA_OK != Tukija::sm_ctrl(irq_sel(), Tukija::SEMAPHORE_DOWN))
...
void Irq_object::start(unsigned irq, addr_t const device_phys, Irq_args const &irq_args)
{
	...
	int ret = map_local(platform_specific().core_pd_sel(),
	                    *(Tukija::Utcb *)Thread::myself()->utcb(),
	...
	auto gsi_flags = [] (Irq_args const &args) {
		if (args.trigger() == Irq_session::TRIGGER_UNCHANGED
		 || args.polarity() == Irq_session::POLARITY_UNCHANGED)
			return Tukija::Gsi_flags();

		if (args.trigger() == Irq_session::TRIGGER_EDGE)
			return Tukija::Gsi_flags(Tukija::Gsi_flags::EDGE);

		if (args.polarity() == Irq_session::POLARITY_HIGH)
			return Tukija::Gsi_flags(Tukija::Gsi_flags::HIGH);
		else
			return Tukija::Gsi_flags(Tukija::Gsi_flags::LOW);
	};
...
Irq_object::~Irq_object()
{
	...
	/* revoke IRQ SM */
	Tukija::revoke(Tukija::Obj_crd(_kernel_caps, KERNEL_CAP_COUNT_LOG2));
