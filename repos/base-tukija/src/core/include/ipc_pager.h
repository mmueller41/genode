...
static inline Tukija::Rights nova_map_rights(Core::Mapping const &mapping)
{
	return Tukija::Rights(true, mapping.writeable, mapping.executable);
}


static inline Tukija::Mem_crd nova_src_crd(Core::Mapping const &mapping)
{
	return Tukija::Mem_crd(mapping.src_addr >> Core::PAGE_SIZE_LOG2,
	                     mapping.size_log2 - Core::PAGE_SIZE_LOG2,
	                     nova_map_rights(mapping));
}


static inline Tukija::Mem_crd nova_dst_crd(Core::Mapping const &mapping)
{
	return Tukija::Mem_crd(mapping.dst_addr >> Core::PAGE_SIZE_LOG2,
	                     mapping.size_log2 - Core::PAGE_SIZE_LOG2,
	                     nova_map_rights(mapping));
}
...
class Core::Ipc_pager
{
	...
	public:
		...
		Ipc_pager (Tukija::Utcb &, addr_t pd_dst, addr_t pd_core);
