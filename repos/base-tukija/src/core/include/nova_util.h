 ...
 */
static int map_local(Genode::addr_t const pd, Tukija::Utcb &utcb,
                     Tukija::Crd const src_crd, Tukija::Crd const dst_crd,
                     ...
                     bool const write_combined = false)
{
	...
	Tukija::uint8_t res = Tukija::delegate(pd, pd, dst_crd);
...
static inline int unmap_local(Tukija::Crd crd, bool self = true) {
	return Tukija::revoke(crd, self); }

inline int map_local_phys_to_virt(Tukija::Utcb &utcb, Tukija::Crd const src,
                                  Tukija::Crd const dst, Genode::addr_t const pd)
{
	return map_local(pd, utcb, src, dst, true);
}

inline int map_local_one_to_one(Tukija::Utcb &utcb, Tukija::Crd const crd,
 ...
 */
inline int map_local(Genode::addr_t const pd, Tukija::Utcb &utcb,
                     Genode::addr_t from_start, Genode::addr_t to_start,
                     Genode::size_t num_pages,
                     Tukija::Rights const &permission,
 ...
 */
inline void unmap_local(Tukija::Utcb &, Genode::addr_t start,
                        Genode::size_t num_pages,
                        bool const self = true,
                        Tukija::Rights const rwx = Tukija::Rights(true, true, true))
{
	using namespace Tukija;
	using namespace Genode;
...
inline Tukija::uint8_t syscall_retry(Core::Pager_object &pager, auto const &fn)
{
	Tukija::uint8_t res;
	do {
		res = fn();
	} while (res == Tukija::NOVA_PD_OOM && Tukija::NOVA_OK == pager.handle_oom());

	return res;
}

inline Tukija::uint8_t async_map(Core::Pager_object &pager,
                               Genode::addr_t const source_pd,
                               Genode::addr_t const target_pd,
                               Tukija::Obj_crd const &source_initial_caps,
                               Tukija::Obj_crd const &target_initial_caps,
                               Tukija::Utcb &utcb)
{
	/* asynchronously map capabilities */
	utcb.set_msg_word(0);
	...
	return syscall_retry(pager,
		[&] {
			return Tukija::delegate(source_pd, target_pd, target_initial_caps);
		});
}

inline Tukija::uint8_t map_vcpu_portals(Core::Pager_object &pager,
                                      Genode::addr_t const source_exc_base,
                                      Genode::addr_t const target_exc_base,
                                      Tukija::Utcb &utcb,
                                      Genode::addr_t const source_pd)
{
	using Tukija::Obj_crd;
	using Tukija::NUM_INITIAL_VCPU_PT_LOG2;
...
inline Tukija::uint8_t map_pagefault_portal(Core::Pager_object &pager,
                                          Genode::addr_t const source_exc_base,
                                          Genode::addr_t const target_exc_base,
                                          Genode::addr_t const target_pd,
                                          Tukija::Utcb &utcb)
{
	using Tukija::Obj_crd;
	using Tukija::PT_SEL_PAGE_FAULT;
...
inline Tukija::Hip const &kernel_hip()
{
	/**
	...
	return *reinterpret_cast<Tukija::Hip const *>(__initial_sp);
}
