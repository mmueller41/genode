...
inline Tukija::uint8_t retry_syscall(addr_t pd_sel, auto const &fn)
{
	Tukija::uint8_t res;
	do {
		res = fn();
	} while (res == Tukija::NOVA_PD_OOM &&
	         Tukija::NOVA_OK == Pager_object::handle_oom(Pager_object::SRC_CORE_PD,
	                                                   pd_sel,
	                                                   "core", "ep",
...
bool Pd_session_component::assign_pci(addr_t pci_config_memory, uint16_t bdf)
{
	return retry_syscall(_pd->pd_sel(), [&] {
		return Tukija::assign_pci(_pd->pd_sel(), pci_config_memory, bdf);
	}) == Tukija::NOVA_OK;
}
...
Pd_session::Map_result Pd_session_component::map(Pd_session::Virt_range const virt_range)
{
	...
	Tukija::Utcb  &utcb      = *reinterpret_cast<Tukija::Utcb *>(Thread::myself()->utcb());
	...
	auto map_memory = [&] (Mapping const &mapping)
	{
		...
		uint8_t err = retry_syscall(_pd->pd_sel(), [&] {
			...
			return Tukija::delegate(pd_core, pd_dst, nova_dst_crd(mapping));
		});

		if (err != Tukija::NOVA_OK) {
...
static State acpi_suspend(State const &request)
{
	...
	auto const result      = Tukija::acpi_suspend(cap_suspend, sleep_type_a,
	                                            sleep_type_b);

	if (result == Tukija::NOVA_OK)
