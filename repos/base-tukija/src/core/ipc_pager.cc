...
Ipc_pager::Ipc_pager(Tukija::Utcb &utcb, addr_t pd_dst, addr_t pd_core)
:
	_pd_dst(pd_dst),
	_pd_core(pd_core),
	_fault_ip(utcb.ip),
...
void Ipc_pager::set_reply_mapping(Mapping const mapping)
{
	...
	_syscall_res = Tukija::delegate(_pd_core, _pd_dst, nova_dst_crd(mapping));
}
...
void Ipc_pager::reply_and_wait_for_fault(addr_t sm)
{
	 ...
	 */
	utcb.set_msg_word((_normal_ipc && _syscall_res != Tukija::NOVA_OK) ? 1 : 0);

	Tukija::reply(myself.stack_top(), sm);
}
