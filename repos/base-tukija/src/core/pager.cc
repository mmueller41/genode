...
struct Pager_thread: public Thread
{
	...
	: Thread(Cpu_session::Weight::DEFAULT_WEIGHT, "pager", 2 * 4096, location)
	{
		...
		reinterpret_cast<Tukija::Utcb *>(Thread::utcb())->crd_xlt = Obj_crd(0, ~0UL);
	}
...
bool Pager_object::_migrate_thread()
{
	...
	try {
		...
		Tukija::revoke(Obj_crd(_selectors, 2));

		/* revoke all exception portals selectors */
		Tukija::revoke(Obj_crd(exc_pt_sel_client()+0x00, 4));
		Tukija::revoke(Obj_crd(exc_pt_sel_client()+0x10, 3));
		Tukija::revoke(Obj_crd(exc_pt_sel_client()+0x18, 1));
		Tukija::revoke(Obj_crd(exc_pt_sel_client()+0x1f, 0));
...
void Pager_object::_invoke_handler(Pager_object &obj)
{
	...
	if (utcb.msg_items() == 1 && utcb.msg_words() == 1 && event == 0xaffe) {
		...
		Tukija::Utcb::Item const &item = *utcb.get_item(0);
		Tukija::Crd const cap(item.crd);
	...
	if (event == ~0UL - 1) {
		...
		if (!obj._state.has_signal_sm()) {
			...
			bool res = Tukija::create_sm(obj.exc_pt_sel_client() + PT_SEL_STARTUP,
...
void Pager_object::_construct_pager()
{
	...
	res = Tukija::create_sm(sel_sm_block_pause(), pd_sel, 0);
	if (res != Tukija::NOVA_OK) {
	...
	res = Tukija::create_sm(sel_sm_block_oom(), pd_sel, 0);
	if (res != Tukija::NOVA_OK) {
	...
	_pd_target(Native_thread::INVALID_INDEX)
{
	 ...
	 */
	addr_t const pd_sel = platform_specific().core_pd_sel();
	uint8_t const res = Tukija::create_sm(exc_pt_sel_client() + SM_SEL_EC,
	                                    pd_sel, 0);
	if (res != Tukija::NOVA_OK)
                                 ...
                                 char const * dst_pd, char const * dst_thread)
{
	...
	Tukija::pd_ctrl_debug(pd_to, limit_before, usage_before);

	if (verbose_oom) {
		addr_t limit_source = 0, usage_source = 0;
		/* request current kernel quota usage of source pd */
		Tukija::pd_ctrl_debug(pd_from, limit_source, usage_source);
	...
	uint8_t res = Tukija::NOVA_PD_OOM;

	if (pd_from != pd_to) {
		/* upgrade quota */
		uint8_t res = Tukija::pd_ctrl(pd_from, Pd_op::TRANSFER_QUOTA,
	...
	if (policy == UPGRADE_PREFER_SRC_TO_DST) {
		if (pd_from != core_pd_sel) {
			res = Tukija::pd_ctrl(core_pd_sel, Pd_op::TRANSFER_QUOTA,
			                    pd_to, QUOTA_TRANSFER_PAGES);
			if (res == Tukija::NOVA_OK)
				return res;
		}
                                ...
                                addr_t reason)
{
	...
	/* check assertions - cases that should not happen on Genode@Tukija */
	enum { NO_OOM_PT = 0UL };
	...
	if (assert) {
		...
		utcb.set_msg_word(0);
		reply(myself.stack_top(), myself.native_thread().exc_pt_sel + Tukija::SM_SEL_EC);
	}
