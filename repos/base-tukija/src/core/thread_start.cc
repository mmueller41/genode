...
{
	unmap_local(Tukija::Obj_crd(native_thread().ec_sel, 1));
	unmap_local(Tukija::Obj_crd(native_thread().exc_pt_sel, Tukija::NUM_INITIAL_PT_LOG2));

	cap_map().remove(native_thread().ec_sel, 1, false);
	cap_map().remove(native_thread().exc_pt_sel, Tukija::NUM_INITIAL_PT_LOG2, false);

	/* revoke utcb */
	Tukija::Rights rwx(true, true, true);
	addr_t utcb = reinterpret_cast<addr_t>(&_stack->utcb());
	Tukija::revoke(Tukija::Mem_crd(utcb >> 12, 0, rwx));
}
...
Thread::Start_result Thread::start()
{
	...
	if (map_local(platform_specific().core_pd_sel(),
	              *reinterpret_cast<Tukija::Utcb *>(Thread::myself()->utcb()),
	                           ...
	                           private Core::Trace::Source
	{
		...
		Info trace_source_info() const override
		{
			...
			uint8_t res = Tukija::ec_time(thread.native_thread().ec_sel, ec_time);
