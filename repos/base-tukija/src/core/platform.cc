	...
	return region_alloc().alloc_aligned(size + (guard_page ? get_page_size() : 0),
		...
		[&] (void *core_local_ptr) {
			...
			int res = map_local(_core_pd_sel, *__main_thread_utcb, phys_addr,
			                    ...
			                    Tukija::Rights(true, true, false), true);
...
static void page_fault_handler()
{
		         ...
		         stack_area_virtual_size()))
	{
		...
		Tukija::Utcb * utcb_fault = reinterpret_cast<Tukija::Utcb *>(utcb_addr_f);
		...
		for (unsigned i = 0; i < last_items; i++) {
			Tukija::Utcb::Item * item = utcb_fault->get_item(i);
			if (!item)
				break;

			Tukija::Crd crd(item->crd);
			if (crd.is_null())
				continue;
	...
	_vm_base(0x1000), _vm_size(0), _cpus(Affinity::Space(1,1))
{
	...
	Tukija::create_sm(Tukija::SM_SEL_EC, core_pd_sel(), 0);
	...
	for_each_location([&](Affinity::Location &location) {
		                      ...
		                      private Trace::Source
		{
			...
			Info trace_source_info() const override
			{
				...
				if (name == "killed") {
					res = Tukija::sc_ec_time(sc_sel, sc_sel,
					                       sc_time, ec_time);
				} else {
					...
					res = Tukija::sc_ctrl(sc_sel, sc_time,
					                    syscall_op);
	                           ...
	                           private Trace::Source
	{
		...
		Info trace_source_info() const override
		{
			...
			if (name == "root") {
				uint8_t res = Tukija::sc_ec_time(ec_sc_sel + 1,
				                               ec_sc_sel,
				                               sc_time,
				                               ec_time);
				if (res != Tukija::NOVA_OK)
				...
				uint8_t res = Tukija::ec_time(ec_sc_sel, ec_time);
				if (res != Tukija::NOVA_OK)
