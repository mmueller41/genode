...
/* Tukija includes */
#include <tukija/capability_space.h>
#include <tukija/native_thread.h>
...
struct Nova_vcpu : Rpc_client<Vm_session::Native_vcpu>, Noncopyable
{
	private:
		...
		inline void _read_nova_state(Tukija::Utcb &);

		inline void _write_nova_state(Tukija::Utcb &);

		addr_t _sm_sel() const {
			return Tukija::NUM_INITIAL_PT_RESERVED + _id_elem.id().value * 4; }
		...
		void _handle_exit(Tukija::Utcb &);

		__attribute__((regparm(1))) static void _exit_entry(addr_t badge);

		Tukija::Mtd _portal_mtd(unsigned exit, Exit_config const &config)
		{
			...
			mtd |= Tukija::Mtd::STA;
			mtd |= Tukija::Mtd::TSC;
			mtd |= Tukija::Mtd::TSC_AUX;
			mtd |= Tukija::Mtd::EFER;
			mtd |= Tukija::Mtd::PDPTE;
			mtd |= Tukija::Mtd::SYSCALL_SWAPGS;
			mtd |= Tukija::Mtd::TPR;
			mtd |= Tukija::Mtd::QUAL;
			mtd |= Tukija::Mtd::XSAVE;
			mtd |= Tukija::Mtd::FPU;

			return Tukija::Mtd(mtd);
		}
...
void Nova_vcpu::_read_nova_state(Tukija::Utcb &utcb)
{
	...
	if (utcb.mtd & Tukija::Mtd::EBSD) {
		_vcpu_state.di.charge(utcb.di);
		_vcpu_state.si.charge(utcb.si);
		_vcpu_state.bp.charge(utcb.bp);
	}

	if (utcb.mtd & Tukija::Mtd::EFL) _vcpu_state.flags.charge(utcb.flags);
	if (utcb.mtd & Tukija::Mtd::ESP) _vcpu_state.sp.charge(utcb.sp);
	if (utcb.mtd & Tukija::Mtd::DR)  _vcpu_state.dr7.charge(utcb.dr7);

	if (utcb.mtd & Tukija::Mtd::EIP) {
		_vcpu_state.ip.charge(utcb.ip);
		_vcpu_state.ip_len.charge(utcb.instr_len);
	}

	if (utcb.mtd & Tukija::Mtd::R8_R15) {
	...
	if (utcb.mtd & Tukija::Mtd::SYSCALL_SWAPGS) {
...
void Nova_vcpu::_write_nova_state(Tukija::Utcb &utcb)
{
	utcb.items = 0;
	utcb.mtd = 0;
	...
	if (_vcpu_state.pdpte_0.charged() || _vcpu_state.pdpte_1.charged() ||
		...
		utcb.mtd |= Tukija::Mtd::PDPTE;
		utcb.pdpte[0] = (Tukija::mword_t)_vcpu_state.pdpte_0.value();
		utcb.pdpte[1] = (Tukija::mword_t)_vcpu_state.pdpte_1.value();
		utcb.pdpte[2] = (Tukija::mword_t)_vcpu_state.pdpte_2.value();
		utcb.pdpte[3] = (Tukija::mword_t)_vcpu_state.pdpte_3.value();
	}
	...
	if (_vcpu_state.star.charged() || _vcpu_state.lstar.charged() ||
		...
		utcb.mtd  |= Tukija::Mtd::SYSCALL_SWAPGS;
	...
	if (_vcpu_state.xcr0.charged() || _vcpu_state.xss.charged()) {
		...
		utcb.mtd |= Tukija::Mtd::XSAVE;
	}

	if (_vcpu_state.fpu.charged()) {
		utcb.mtd |= Tukija::Mtd::FPU;
 ...
 */
void Nova_vcpu::_handle_exit(Tukija::Utcb &utcb)
{
...
void Nova_vcpu::with_state(auto const &fn)
{
	...
	Tukija::Utcb &utcb = *reinterpret_cast<Tukija::Utcb *>(myself->utcb());
	...
	if (remote) {
		...
		Exit_config config { };
		Tukija::Mtd mtd = _portal_mtd(0, config);

		uint8_t res = Tukija::ec_ctrl(Tukija::EC_GET_VCPU_STATE, _ec_sel(), mtd.value());
	...
	if (remote) {
		...
		uint8_t res = Tukija::ec_ctrl(Tukija::EC_SET_VCPU_STATE, _ec_sel(), recall);
		 ...
		 */
		if (_resume)
			Tukija::sm_ctrl(_sm_sel(), Tukija::SEMAPHORE_UP);
	}
}


static void nova_reply(Thread &myself, Tukija::Utcb &utcb, auto &&... args)
{
	...
	Tukija::reply(myself.stack_top(), args...);
}
                                                          ...
                                                          Tukija::Mtd          mtd)
{
	...
	Tukija_native_pd_client native_pd { pd.native_pd() };
	...
	_id_elem(*this, _vcpu_space()), _obj(handler), _alloc(alloc)
{
	...
	Signal_context_capability dontcare_exit =
		_create_exit_handler(env.pd(), handler, vcpu_id, 0x100, Tukija::Mtd(Tukija::Mtd::EIP));

	for (unsigned i = 0; i < Tukija::NUM_INITIAL_VCPU_PT; ++i) {
		Signal_context_capability signal_exit;

		Tukija::Mtd mtd = _portal_mtd(i, exit_config);
		if (mtd.value()) {
