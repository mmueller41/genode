...
#include <tukija_native_cpu/client.h>
...
void Thread::_init_platform_thread(size_t weight, Type type)
{
	...
	if (type == MAIN) {
		...
		native_thread().ec_sel     = Tukija::EC_SEL_THREAD;
...
Thread::Start_result Thread::start()
{
	...
	try {
		...
		Tukija_native_cpu_client native_cpu(_cpu_session->native_cpu());
	...
	native_thread().ec_sel = native_thread().exc_pt_sel + Tukija::EC_SEL_THREAD;

	/*
	 * Requested ec cap that is used for recall and
	 * creation of portals (Native_pd::alloc_rpc_cap).
	 */
	request_native_ec_cap(native_thread().exc_pt_sel + Tukija::PT_SEL_PAGE_FAULT,
