                                    ...
                                    size_t rcv_caps)
{
	...
	Tukija::Utcb &utcb = *(Tukija::Utcb *)myself->utcb();
	...
	if (keep && !reinit) {
		for (unsigned i = 0; i < MAX_CAP_ARGS; i++) {
			...
			if (_rcv_pt_cap_free[i] == UNUSED_CAP)
				Tukija::revoke(Tukija::Obj_crd(_rcv_pt_base + i, 0), true);
...
bool Receive_window::prepare_rcv_window(Tukija::Utcb &utcb, addr_t rcv_window)
{
	/* open maximal translate window */
	utcb.crd_xlt = Tukija::Obj_crd(0, ~0UL);
	...
	if (rcv_window != INVALID_INDEX) {
		...
		/* open receive window */
		utcb.crd_rcv = Tukija::Obj_crd(_rcv_pt_base, _rcv_wnd_log2);
		return true;
	}
