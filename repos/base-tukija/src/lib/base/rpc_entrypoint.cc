...
static void cleanup_call(Rpc_object_base *obj, Tukija::Utcb * ep_utcb,
...
void Rpc_entrypoint::_dissolve(Rpc_object_base *obj)
{
	...
	Tukija::revoke(Tukija::Obj_crd(obj->cap().local_name(), 0), true);

	/* make sure nobody is able to find this object */
	remove(obj);

	cleanup_call(obj, reinterpret_cast<Tukija::Utcb *>(this->utcb()), _cap);
}

static void reply(Tukija::Utcb &utcb, Rpc_exception_code exc, Msgbuf_base &snd_msg)
{
	copy_msgbuf_to_utcb(utcb, snd_msg, exc.value);

	Tukija::reply(Thread::myself()->stack_top());
}
...
void Rpc_entrypoint::_activation_entry()
{
	...
	Tukija::Utcb     &utcb = *(Tukija::Utcb *)Thread::myself()->utcb();
	...
	if (!rcv_window.prepare_rcv_window(*(Tukija::Utcb *)ep.utcb()))
		warning("out of capability selectors for handling server requests");
	...
	_pd_session(*pd_session)
{
	...
	if (!rcv_window.prepare_rcv_window(*(Tukija::Utcb *)&_stack->utcb()))
