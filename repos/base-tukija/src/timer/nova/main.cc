		...
		struct Waiter : Thread
		{
			struct Sel /* NOVA kernel-capability selector */
			{
				...
				static Sel init_signal_sem(Thread &thread)
				{
					...
					request_signal_sm_cap(exc_base + Tukija::PT_SEL_PAGE_FAULT,
					                      exc_base + Tukija::SM_SEL_SIGNAL);

					return { exc_base + Tukija::SM_SEL_SIGNAL };
				}

				auto down(Tsc deadline)
				{
					return Tukija::sm_ctrl(value, Tukija::SEMAPHORE_DOWN, deadline.tsc);
				}

				auto up()
				{
					return Tukija::sm_ctrl(value, Tukija::SEMAPHORE_UP);
				}
			};
			...
			void entry() override
			{
				...
				for (;;) {
					 ...
					 */
					if (_wakeup_sem.down(deadline_tsc()) == Tukija::NOVA_TIMEOUT)
			...
			void update_deadline(Tsc const deadline)
			{
				...
				if (sooner_than_scheduled)
					if (_wakeup_sem.up() != Tukija::NOVA_OK)
