 ...
 * than 0.
 */
static inline Tukija::mword_t copy_utcb_to_msgbuf(Tukija::Utcb             &utcb,
 ...
 */
static inline bool copy_msgbuf_to_utcb(Tukija::Utcb                &utcb,
                                       Genode::Msgbuf_base const &snd_msg,
                                       Tukija::mword_t              protocol_value)
{
	using namespace Genode;
	using namespace Tukija;
	...
	for (unsigned i = 0; i < snd_msg.used_caps(); i++) {
		...
		Tukija::Crd const crd = Capability_space::crd(cap);
