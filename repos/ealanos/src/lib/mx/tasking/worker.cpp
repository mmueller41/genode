...
Worker::Worker(const std::uint16_t id, const std::uint16_t target_core_id, const std::uint16_t target_numa_node_id,
    ...
    : _target_core_id(target_core_id), _target_numa_node_id(target_numa_node_id),  _prefetch_distance(prefetch_distance),
...
void Worker::execute()
{
	...
	if (_id == 0)
		Genode::log("Foreman uses core_id", core_id);
    while (true)
    {
        handle_resume();
        ...
        while ((task = this->current->next_task()) != nullptr)
        {
            ...
            if (result.is_remove())
            {
                runtime::delete_task(channel_id, task);
            }
...
void Worker::deregister() {
    runtime::scheduler().deregister_worker();
}
