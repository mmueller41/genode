...
public:
     ...
     */
    static bool init(Libc::Env &env, const util::core_set &core_set, const std::uint16_t prefetch_distance,
                     const bool use_system_allocator)
    {

		if (mx::memory::GlobalHeap::_alloc == nullptr) {
			mx::memory::GlobalHeap::_alloc = new mx::memory::Alloc(env.pd(), env.rm());
			mx::system::Environment::set_env(&env);
			mx::system::Environment::get_platform_info();
		}
        ...
        else
        {
            ...
            _task_allocator.reset(new (
                memory::GlobalHeap::allocate_cache_line_aligned(sizeof(memory::fixed::Allocator<config::task_size()>)))
                                      memory::fixed::Allocator<config::task_size()>());
        }
	...
	static void reset_usage_predictions() { _scheduler->reset_usage_predictions(); }
	/**
     * Creates a new task.
...
public:
    ...
    {
        runtime::init(env, core_set, prefetch_distance, use_system_allocator);
    }

    runtime_guard(Libc::Env &env, const util::core_set &core_set, const std::uint16_t prefetch_distance = 0U) noexcept
